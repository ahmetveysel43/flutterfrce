// lib/presentation/screens/vald_test_flow_screen.dart - FIXED
...
class _ValdTestFlowScreenState extends State<ValdTestFlowScreen>
  ...
  void _setupInitialData() {
    // Setup listeners and initialize data
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      // Load athletes into flow controller
      flowController.loadAthletes(athleteController.athletes);
      
      // Setup USB auto-connection
      _setupUsbAutoConnection();
      
      // Setup force data forwarding
      _setupForceDataForwarding();
    });
  }

  void _setupUsbAutoConnection() {
    final usbController = context.read<UsbController>();
    final flowController = context.read<ValdTestFlowController>();
    
    // Listen to USB connection changes
    usbController.addListener(() {
      if (usbController.isConnected && !flowController.isConnected) {
        flowController.connectToDevice(
          usbController.connectedDeviceId ?? 'Mock Platform'
        );
      } else if (!usbController.isConnected && flowController.isConnected) {
        flowController.disconnect();
      }
    });
    
    // Auto-scan for devices if none available
    if (usbController.availableDevices.isEmpty) {
      usbController.refreshDevices().then((_) {
        if (usbController.availableDevices.isNotEmpty) {
          usbController.connectToDevice(usbController.availableDevices.first);
        }
      });
    }
  }

  void _setupForceDataForwarding() {
    final usbController = context.read<UsbController>();
    final flowController = context.read<ValdTestFlowController>();
    
    // Listen to force data stream and forward to flow controller
    usbController.forceDataStream?.listen((forceData) {
      flowController.addTestData(forceData);
    });
  }
          ...
          children: [
            ...
            Row(
              ...
              children: [
                OutlinedButton.icon(
                  onPressed: () {
                    context.read<ValdTestFlowController>().goToPreviousStep();
                ...
                ElevatedButton.icon(
                  onPressed: () {
                    context.read<ValdTestFlowController>().restartFlow();
                    _stepAnimationController.reset();
                    _stepAnimationController.forward();
                  },
        ...
        child: Column(
          ...
          children: [
            ...
            ListTile(
              ...
              subtitle: Text(usbController.connectionStatusText),
              onTap: () {
                Navigator.pop(context);
            ...
            if (flowController.currentStep == ValdTestStep.testing)
              ListTile(
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('Status: ${usbController.connectionStatusText}'),
            const SizedBox(height: 8),
            Text('Available Devices: ${usbController.availableDevices.length}'),
          ...
          if (!usbController.isConnected)
            ElevatedButton(
              onPressed: () async {
                ...
                if (usbController.availableDevices.isNotEmpty) {
                  await usbController.connectToDevice(
                    usbController.availableDevices.first
                  );
                }
              },
