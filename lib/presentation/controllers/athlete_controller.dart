// lib/presentation/controllers/athlete_controller.dart - FIXED VERSION
import 'dart:async';
import 'dart:convert';
...
import '../../core/constants/enhanced_test_protocols.dart'; // ✅ FIXED: Added import for TestType
...
extension AthleteControllerExtensions on AthleteController {
  ...
  /// Get recommended tests for athlete - ✅ FIXED: Now TestType is properly imported
  List<TestType> getRecommendedTests(Athlete athlete) {
    ...
    if (athlete.injuryHistory.isNotEmpty) {
      recommendations.add(TestType.balance);
      recommendations.add(TestType.injuryRisk);
      // Potential recommendation to avoid certain tests based on injury type
    }
    
    return recommendations.toSet().toList(); // ✅ FIXED: Use toSet().toList() instead of distinct()
  }
}