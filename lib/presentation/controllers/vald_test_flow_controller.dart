// lib/presentation/controllers/vald_test_flow_controller.dart - FIXED
import 'dart:async';
...
/// VALD ForceDecks benzeri 7 adımlı test akışını yöneten controller - FIXED
class ValdTestFlowController extends ChangeNotifier {
  ...
  // Step 6: Real-time Testing - FIXED
  void startTest() {
    ...
    _testTimer?.cancel(); // Cancel any existing timer
  ...
  void _stopTest() {
    ...
    _testTimer?.cancel(); // ✅ FIXED: Cancel timer
    _calculateResults();
  ...
  // Step 7: Results Calculation - IMPROVED
  void _calculateResults() {
    ...
    final asymmetries = _testData.map((d) => d.asymmetryIndex).toList();
    final avgAsymmetry = asymmetries.isNotEmpty ? asymmetries.reduce((a, b) => a + b) / asymmetries.length : 0.0;
    
    // Jump height calculation (simplified)
    final bodyWeightN = (_measuredWeight ?? 70.0) * 9.81;
    double jumpHeight = 0.0;
    
    if (peakForce > bodyWeightN) {
      // Simplified jump height based on force-time data
      final impulse = forces.where((f) => f > bodyWeightN).fold(0.0, (sum, f) => sum + (f - bodyWeightN)) / 1000;
      jumpHeight = (impulse * impulse) / (2 * bodyWeightN * 9.81) * 100; // cm
      jumpHeight = jumpHeight.clamp(0, 100); // Reasonable limits
    }
    ...
    _testResults = {
      ...
      'bodyWeight': _measuredWeight ?? 0.0,
      'asymmetryIndex': avgAsymmetry * 100, // Convert to percentage
    };
  }
  ...
  void _resetFlow() {
    ...
    _testData.clear();
    _testResults = null;
    _testTimer?.cancel(); // ✅ FIXED: Cancel any running timer
    _clearError();
  }
