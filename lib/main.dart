// lib/main.dart - FIXED NAVIGATION
...
import 'presentation/controllers/vald_test_flow_controller.dart'; // ✅ ADDED
  ...
  Widget build(BuildContext context) {
    return MultiProvider(
      providers: [
        ...
        // ✅ ADDED: VALD Test Flow Controller Provider
        ChangeNotifierProvider<ValdTestFlowController>(
          create: (_) => ValdTestFlowController(),
        ),
      ],
      child: MaterialApp(
...
/// App loading screen - FIXED NAVIGATION
...
class _AppLoadingScreenState extends State<AppLoadingScreen>
  ...
  Future<void> _performAppInitialization() async {
    try {
      ...
      // Load athletes in background
      try {
        await sl<AthleteController>().loadAthletes();
        debugPrint('✅ Athletes loaded successfully');
      } catch (e) {
        debugPrint('❌ Athletes loading failed: $e');
      }
      
      // Initialize USB mock data
      try {
        sl<UsbController>().setMockPersonOnPlatform(false);
        debugPrint('✅ USB controller initialized');
      } catch (e) {
        debugPrint('❌ USB controller initialization failed: $e');
      }
      
      // Navigate to main screen with proper provider setup
      if (mounted) {
        Navigator.of(context).pushReplacement(
          MaterialPageRoute(
            builder: (context) => MultiProvider(
              providers: [
                // Pass existing providers down
                ChangeNotifierProvider<UsbController>.value(
                  value: sl<UsbController>(),
                ),
                ChangeNotifierProvider<AthleteController>.value(
                ...
                // Create new VALD test flow controller
                ChangeNotifierProvider<ValdTestFlowController>(
                  create: (_) => ValdTestFlowController(),
                ),
              ],
              child: const ValdTestFlowScreen(),
            ),
          ),
        );
      }
